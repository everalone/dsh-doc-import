...
export const zh = {
  'button.title': '导入文档',
  'button.aria': '导入 txt / md / csv / docx / pdf 文档，自动解析并内联进消息',
  'chip.uploading': '上传中…',
  'chip.parsing': '解析中…',
  'chip.ocr': 'OCR 识别中 {done}/{total} 页',
  'chip.ready': '已就绪 · {chars} 字符',
  'chip.truncated': '已就绪 · 内联 {chars} 字符（全文 {total} 字符，可回读）',
  'chip.error': '失败：{error}',
  'chip.remove': '移除文档',
  'chip.cost': '预计 {label}',
  'toast.cost': '本次内联文档约 {chars} 字符 ≈ {tokens} tokens ≈ {cny}（按当前模型输入价估算，非阻断提示）',
  'toast.ocr': '含 OCR：约 {ocrCny}',
  'drop.title': '导入文档',
  'drop.hint': '松开即可导入 txt / md / csv / docx / pdf（扫描版 PDF 自动 OCR）',
  'card.title': '文档导入',
  'card.description': '拖入 txt / md / csv / docx / pdf 后的解析、存储与 OCR 行为；修改即时生效。',
  'card.loading': '加载设置中…',
...
export const en: Record<DocImportClientKey, string> = {
  ...
  'button.aria': 'Import txt / md / csv / docx / pdf documents; content is parsed and inlined automatically',
  'chip.uploading': 'Uploading…',
  'chip.parsing': 'Parsing…',
  'chip.ocr': 'OCR {done}/{total} pages',
  ...
  'chip.cost': 'Est. {label}',
  'toast.cost': 'Inlining {chars} chars ≈ {tokens} tokens ≈ {cny} (input-price estimate, non-blocking)',
  'toast.ocr': 'incl. OCR: ~{ocrCny}',
  ...
  'card.description': 'Parsing, storage and OCR behavior for dragged txt / md / csv / docx / pdf documents; edits apply immediately.',
