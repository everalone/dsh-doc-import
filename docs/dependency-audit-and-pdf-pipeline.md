...
## 四、实测事故分析（2026-09-05 会话 a756dd04，已全部修复）

真实导入《文献综述-SY2410208-沈家鹏.docx》后暴露三个连锁问题：

1. **模型全盘 grep 找文档 id，会话卡死**
   根因：引用头只有 `[document …, id: <sha256>]`，没有告诉模型 id 该怎么用；而
   `read_document` 工具描述的触发条件又限定在「内联截断/已截断」头上——纯引用模式下
   条件不匹配，模型把 id 当文件路径，`grep -R` 全盘搜索直到被终止。
   修复：引用头自带读取指引（"如需全文，调用 read_document 工具并传入上面的 id；
   id 不是文件路径，在磁盘上搜不到"）；工具描述放宽为"凡出现 `[document …, id: …]`
   头即应调用本工具，禁止在文件系统搜索 id"。

2. **提取文本出现大段"乱码"（base64）**
   根因：那不是乱码，是 docx 内嵌图片（Word 的"徽标"与"AI 生成内容"提示图标）被
   mammoth 默认转换为 `data:image/png;base64,…` 内联进正文。实测：全文 158,329 字符
   中 124,552 字符（78.7%）是 2 张图片的 base64，另有 77 个空书签锚点。
   修复（`sanitizeDocxMarkdown`）：图片 data URI → `【图片】` 占位符；清除空锚点；
   折叠多余空行；warnings 报告图片数。实测同一文件：158,329 → 29,740 字符，
   base64 清零，正文完整。`EXTRACTOR_VERSION` 3→4，旧文档重导入自动重新提取。

3. **子代理疯狂复读 /f/f/f…**
   根因与 #2 相同：子代理读入含 78.7% 随机 base64 的文本后，模型退化为复读随机
   字符串（LLM 对长随机序列的典型崩塌），只能终止。图片占位符修复后该诱因消除。

**附带改进**：老版 `.doc`（Word 97-2003，OLE2 格式，mammoth 无法解析）此前只会得到
泛泛的"不支持"报错，现在给出明确指引（另存为 .docx 或导出 PDF）。
