...
- **基准版本**：`main` @ `5799f3d`（v1.1 + 引用头路径补丁 + 三次客户端修复），MIT，仓库 `everalone/dsh-doc-import`
...
- **未验证项**：仅剩一项可选观察——梁神会话首轮贴文档时，模型是否直接 `cat <路径>`
  （用户已复测通过：贴文档 → 卡片自动就绪 → 切会话后仍可发送 → 消息带引用、引导语不再显示）
...
### 2026-09-10 · 知识库体检（neat-freak）

- **README**：Node 要求 18+ → `^22.19.0 || >=24.0.0`（与 package engines 对齐）；删掉指向
  已移入本地 `review/` 的失效文档链接，改为列 `docs/PROGRESS.md`；补两条新特性
  （引用行自带 shell 路径、草稿跨会话/刷新保留）；测试描述补客户端用例。
- **包内 README**：从"host 端自动解析文本并内联进用户消息"的旧描述，改写为当前
  "紧凑引用 + 文件卡片 + read_document 回读"模型，并补三个文档指针。
- **roadmap**：补齐 v1.1 已完成项（官方环境一等公民、OCR 自愈与终态、read_document 分页、
  提取器 3/4、引用行 shell 路径、客户端健壮性），新增"延期格式（doc/xlsx/pptx/rtf）"
  与"发布 npm 一键安装（用户暂缓）"两条待办。
- **CONTEXT.md**：收敛为纯词汇表 + 决策索引（新增术语：引用行 / 文件卡片 / 草稿），
  决策与进度统一指向 `docs/adr/`、`PROGRESS.md`、`roadmap.md`，消除三处重复维护。
- **新增 ADR 0003**：草稿全局且持久（取代早先"按会话隔离"的实现选择）。
- **AGENTS.md**：单测数 22 → 31；目录速查补 `test/client-state.test.mjs`；修正失效的章节
  引用；禁止事项补"禁止在会话切换/重载时清空草稿"；相对时间措辞改为中性表述。
- **review/project-intro.zh.md**（本地、不入库）：更新到 v1.1 + 今日四项修复
  （shell 路径、草稿丢失、快照不重渲染、引导语泄漏 UI）。
- **未动**：ADR 0001/0002 的历史表述（ADR 是历史记录，不改写）；各处历史日志条目里的
  数字（如"22 项单测"是当次事实，当前状态以本文件顶部为准）。

### 2026-09-10 · 用户复测通过（收尾）

- 用户硬刷新后复测：**贴文档 → 卡片自动变"已就绪"（无需敲字）→ 切到新对话草稿仍在 →
  发送后消息带 `[document …]` 引用 → 对话里看不到引导语行（tooltip 仍可查）**；"目前测试没有问题"。
- 本次会话累计修复三处客户端缺陷（草稿被会话切换清空、快照引用不变导致不重渲染、引导语误显示），
  均带回归测试并写入 `AGENTS.md` 已知坑第 8/9 条。
- 未闭合项：梁神首轮"`cat <路径>` 而非全盘 grep"属可选观察，下次遇到时顺手确认即可。
