...
- 流程（ADR 0002 取代 ADR 0001 后）：拖入 → 文档导入 → host 端解析 → 消息只携带紧凑文件引用；扫描版 PDF 自动 OCR 回退，无需用户干预。
...
- OCR 护栏：导入后自动执行；单文档 OCR 页数上限默认 100 页（0 = 不限），超出的页跳过并警告；页级并发 3，失败重试 1 次；进度实时显示，作业异常必有终态（就绪或失败），不会挂起发送。
